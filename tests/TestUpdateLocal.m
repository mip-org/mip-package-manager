...
%     - source_path is stored for local installs (both editable and copy)
    ...
    methods (Test)
        ...
        function testUpdateLocalPackage_NoCompileFlagSkipsCompilation(testCase)
            ...
            mip.update('--no-compile', 'local/mypkg');
        ...
        function testUpdateLocalPackage_NoCompileWithForce(testCase)
            ...
            mip.update('--no-compile', '--force', 'local/mypkg');
        ...
        function testUpdateLocalPackage_UnloadedStaysUnloaded(testCase)
            ...
            testCase.verifyFalse(mip.state.is_loaded('local/mypkg'));
        ...
        function testUpdateLocalPackage_EmptySourcePathSkipped(testCase)
            ...
            output = evalc("mip.update('local/urlpkg')");
            testCase.verifyTrue(contains(output, 'Skipping'));
        ...
        function testUpdateForce_EmptySourcePathSkipped(testCase)
            ...
            output = evalc("mip.update('--force', 'local/urlpkg')");
            testCase.verifyTrue(contains(output, 'Skipping'));
        end
        ...
        function testUpdateAll_SkipsEmptySourcePathPackages(testCase)
            ...
            normalPkgDir = fullfile(testCase.TestRoot, 'packages', 'local', 'normalpkg');
        ...
        function testUpdateLocalPackage_PreservesDirectlyInstalled(testCase)
            ...
            testCase.verifyTrue(ismember('local/mypkg', ...
        ...
        function testUpdateDeps_WithForce(testCase)
            ...
            mip.update('--deps', '--force', 'local/mypkg');
