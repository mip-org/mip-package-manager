...
%TESTPARSEYAML   Tests for mip.parse.parse_yaml.
    ...
    methods (Test)
        ...
        function testNestedBlockSequence(testCase)
            ...
            testCase.verifyEqual(length(r.outer{1}.paths), 2);
            testCase.verifyEqual(r.outer{1}.paths{1}.path, 'a');
            testCase.verifyEqual(r.outer{1}.paths{2}.path, 'b');
        end
        ...
        function testRealisticMipYaml(testCase)
            ...
            testCase.verifyEqual(length(r.paths), 2);
            testCase.verifyEqual(r.paths{1}.path, 'matlab');
            testCase.verifyEqual(r.paths{2}.path, 'matlab/numbl');
            ...
            testCase.verifyEqual(r.builds{2}.compile_script, 'compile_numbl_wasm.m');
        end
