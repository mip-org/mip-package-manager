function localPath = download_file(source, destDir)
    ...
    %
    % Example:
    %   filepath = mip.utils.download_file('https://example.com/pkg.mhl', tempdir);
    %   filepath = mip.utils.download_file('/path/to/local/pkg.mhl', tempdir);
    ...
    isURL = startsWith(source, 'http://') || startsWith(source, 'https://');
    ...
    if isURL
        ...
        if ~exist(source, 'file')
            error('mip:fileNotFound', ...
                  'Local file not found: %s', source);
        end
